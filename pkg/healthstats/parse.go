...
import (
	...
	healthmetrics "github.com/robotjoosen/go-health-service/pkg/metrics"
...
// messages (health-service's own exported healthmetrics.SysUsageMessage shape) into the
...
func ParseHealthMessage(raw []byte) (host string, stats domain.HostStats, err error) {
	var msg healthmetrics.SysUsageMessage
	if err := json.Unmarshal(raw, &msg); err != nil {
		return "", domain.HostStats{}, err
	}

	temperatures := make([]domain.Temperature, 0, len(msg.Tmp))
	for _, t := range msg.Tmp {
